...
hmc_sampler <- function(U,dUdq,M,eps_gen,L) {
  .check_args(U=U, dUdq = dUdq, M=M, eps_gen = eps_gen, L=L)
  cholM <- chol(M)
  Minv <- chol2inv(cholM)
  function(q) {
    stopifnot(length(q) == nrow(M))
    .hmc_sample (q, U=U, dUdq = dUdq, Minv = Minv, cholM = cholM, eps_gen=eps_gen, L=L)
  }
}
...
#' 
#' plot(qp$q,qp$p)
leapfrog <- function(qp,dUdq,Minv,eps,L=1L) {
...
.propose_state <- function(qp,U,dUdq,Minv,eps,L) {
  
  #Run the dynamics
  qp <- leapfrog(qp = qp ,dUdq = dUdq,Minv = Minv,eps = eps,L=L)
...
.hmc_sample <- function(q,U,dUdq,Minv,cholM,eps_gen,L) {
  ...
  proposed_qp <- .propose_state(qp,U = U,dUdq = dUdq,Minv = Minv,eps = eps_gen(),L = L)
